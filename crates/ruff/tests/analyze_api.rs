...
/// Test analyzing a single module with the short flag
#[test]
fn analyze_single_module_short() -> Result<()> {
    let tempdir = TempDir::new()?;
    let root = tempdir.path();

    setup_test_package(root)?;

    // Run the API analysis on the core module with --short flag
    insta::with_settings!({
        filters => INSTA_FILTERS.to_vec(),
        snapshot_suffix => "short_flag",
    }, {
        assert_cmd_snapshot!(
            command()
                .arg(root.join("mypackage").join("core.py"))
